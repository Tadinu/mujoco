#ifndef MUJOCO_SRC_OPTIMIZATION_PRIMITIVE_H
#define MUJOCO_SRC_OPTIMIZATION_PRIMITIVE_H
...
public:
    ...
    double mu;
    PrimitiveType type;
    Vec3d center, centerInit;
    double mass;
    Vec3d velocity, velocityInit; // center is relative to parent primitive, if there is a parent
    bool isEnabled;
    bool gravityEnabled;
    ...
    static Vec3d getSpherePos(double radius, double phi, double theta) {
      double x = 0;//radius * glm::cos(glm::radians(phi)) * glm::sin(glm::radians(theta));
      double y = 0;//radius * glm::sin(glm::radians(phi)) * glm::sin(glm::radians(theta));
      double z = 0;//radius * glm::cos(glm::radians(theta));
      return Vec3d(x, y, z);
    }
...
#endif //MUJOCO_SRC_OPTIMIZATION_PRIMITIVE_H
