...
std::pair<std::vector<Vec9d>, Mat9x9d> Triangle::stretchingForceAndJacobianCheck() const {
  ...
  Eigen::Matrix<double, 3,9> Du = Utils::kronecker<1,3,3,3>(du , I_three),
          Dv = Utils::kronecker<1,3,3,3>(dv, I_three);
...
Mat6x9d Triangle::projectToManifoldBackward(const VecXd &x_vec) const {
    ...
    dp_dxvec << -1, 1, 0,
            -1, 0, 1;
    Mat6x9d dp_dx = Utils::kronecker<2, 3, 3, 3>(dp_dxvec, I_three); //checked
    ...
    Mat4x4d dF_dF1 = Utils::kronecker<2, 2, 2, 2>(inv_deltaUV.transpose(), I_two);
    ...
    dF2_dR.setZero();
    dF2_dnewdeltaUV = Utils::kronecker<2, 2, 3, 3>(R.transpose(), I_three);
...
void Triangle::projectBackward(const VecXd &x_vec, TripleVector &triplets) {
  Utils::insertIntoTriplets(triplets, newF, 6, 3, 0, 0, c_idx, p0()->idx * 3);
  Utils::insertIntoTriplets(triplets, newF, 6, 3, 0, 3, c_idx, p1()->idx * 3);
  Utils::insertIntoTriplets(triplets, newF, 6, 3, 0, 6, c_idx, p2()->idx * 3);
...
//
  ...
  dF_prime_dR.setZero();
  dF_prime_dnewdeltaUV = Utils::kronecker<2, 2, 3, 3>(R.transpose(), I_three);
...
Triangle::Triangle(int p0_idx, int p1_idx, int p2_idx, std::vector<Particle> &pArr, bool checkArea) : Constraint(
  ...
  p.row(0) = Vec3d(-1, 1, 0);
  p.row(1) = Vec3d(-1, 0, 1);
  dF_dx = Utils::kronecker<2, 3, 3, 3>(inv_deltaUV.transpose() * p, I_three);
  setConstraintWeight();
