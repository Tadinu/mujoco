#ifndef MUJOCO_SRC_OPTIMIZATION_SIMULATION_H
#define MUJOCO_SRC_OPTIMIZATION_SIMULATION_H
...
public:
    ...
    struct BackwardInformation {
        VecXd dL_dx = VecXd(0);
        VecXd dL_dv = VecXd(0);
        VecXd dL_dconstantForceField = VecXd(0);
        Vec3d dL_dfext = Vec3d(0, 0, 0);
        Vec5d dL_dwind = MatXd::Zero(5, 1); // vec3(force),period,phase
        VecXd dL_dwindtimestep = VecXd(0);
        double dL_ddensity = 0.0;
        double dL_dk_pertype[Constraint::CONSTRAINT_NUM] = {0.0, 0.0, 0.0, 0.0};
        std::vector<std::vector<VecXd>> dL_dsplines;
        VecXd dL_dxfixed  = VecXd(0);
        VecXd dL_dxfixed_accum  = VecXd(0);
        std::vector<std::pair<int, double>> dL_dmu = {{0, 0}};
        int badMatrixCounter = 0;
        int goodMatrixCounter = 0;
        double loss = 0;
        ...
        int convergedAccum = 0;
        int backwardIters = 0;
        int backwardTotalIters = 0;
        int correspondingForwardIdxInStats = 0;
        double rho = 0;
    };
...
//    static AttachmentConfigs restPosConfiguration;
    ...
    Sphere slopeGoal;
    LowerLeg sockLeg, rightLeg, leftLeg;
    Capsule leftUpperArm, rightUpperArm, leftLowerArm, rightLowerArm, body, neck, head;
    ...
    void resetForwardRecordsFromFolder(std::string subFolder) {
      ...
      std::vector<std::string>  frameFileNames = Utils::listFiles(fullFolderPath, ".obj");
    ...
    void rotatePointsAccordingToConfig(std::vector<Vec3d> &particleIn) {
      switch (sceneConfig.orientation) {
        case Orientation::CUSTOM_ORIENTATION: {
          ...
          particleIn = rotatePointsAroundCenter(particleIn, Utils::axisToRotation(sceneConfig.upVector, Vec3d(0, 1, 0)));
          break;
        }
        ...
        case Orientation::DOWN: {
          ...
          particleIn = rotatePointsAroundCenter(particleIn, Utils::axisToRotation(Vec3d(0, 1, 0), Vec3d(0, 0, 1)));
          break;
        }

        case Orientation::BACK: {
          std::printf("orientation down: rotate\n");
          particleIn = rotatePointsAroundCenter(particleIn, Utils::axisToRotation(Vec3d(0, 0, 1), Vec3d(1, 0, 0)) *
                                                            Utils::axisToRotation(Vec3d(1, 0, 0), Vec3d(0, 0, -1)));
          break;
        }
    ...
    void writeMatrix(std::ofstream &myfile, MatXd &A, std::string name, int precision) {
      ...
      for (int i = 0; i < A.rows(); i++) {
        for (int j = 0; j < A.cols(); j++) {
          ...
          myfile << Utils::d2str(A(i, j), precision) << " ";
        }
        myfile << "\n";
      }
    }
    ...
    void writeMatrix(std::ofstream &myfile, SpMat &A, std::string name, int precision) {
      ...
      for (int i = 0; i < A.rows(); i++) {
        for (int j = 0; j < A.cols(); j++) {
          ...
          myfile << Utils::d2str(Adense(i, j), precision) << " ";
        }
        myfile << "\n";
      }
    }
    ...
    void writeMatrixSparse(std::ofstream &myfile, SpMat &A, std::string name, int precision) {
      ...
      for (int k = 0; k < A.outerSize(); ++k) {
        for (SpMat::InnerIterator it(A, k); it; ++it) {
          ...
          myfile << r << " " << c << " " << Utils::d2str(elem, precision) << "\n";
        }
      }
    }
    ...
    forwardInfoToString(BackwardTaskInformation taskInfo, const Simulation::ForwardInformation &forwardInfo) {
      ...
      out += "Forward Total Runtime[ms]:" + Utils::d2str((double) (forwardInfo.totalRuntime * 1.0 / 1000.0), 5) + "\n";
      out += "Loss:" + Utils::d2str(forwardInfo.loss, 5) + "\n";
      ...
      for (const TimerEntry &breakdown: forwardInfo.accumTimer) {
        ...
        out += label + "[ms]:" + Utils::d2str(duration / 1000.0, 5) + "\t\t|" + Utils::d2str(double(percentage * 100), 3) + "%\n";
      }
    ...
    void exportStatistics(Demos demoId, TaskSolveStatistics &statistics, BackwardTaskInformation taskInfo,
                          bool writePerf = false);


    void exportOptimizationRecords(Demos demoId, std::string experimentNameGiven = "", bool useGiven = false);


    void exportConfig(Demos demoId, SceneConfiguration &config, std::string fileName);
    ...
    void exportFrameInfo(Simulation *system, Simulation::ForwardInformation &record, std::string fileName) {
      ...
      for (int fixedPointId = 0; fixedPointId < record.x_fixedpoints.rows() / 3; fixedPointId++) {
        ...
        myfile << "CLIP_" << fixedPointId << ":" << Utils::d2str(clipPos[0], 5) << "," << Utils::d2str(clipPos[1], 5) << ","
               << Utils::d2str(clipPos[2], 5) << "\n";
...
#endif //MUJOCO_SRC_OPTIMIZATION_SIMULATION_H
