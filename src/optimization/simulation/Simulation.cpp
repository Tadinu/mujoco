    ...
    for (const PrimitiveCollisionInformation &info : infos.first.first) {
      if (info.primitiveId != -1) {
        ...
        Primitive *prim = primitives[info.primitiveId];
          ...
          Utils::insertIntoTriplets<3, 3>(dr_dftriplets, dri_dd, 3 * pIdx, 3 * pIdx);
    ...
    if (selfcollisionEnabled) {
      ...
      for (const std::vector<SelfCollisionInformation> &selfInfos: infos.second) { // has to use second, because 1. layer matters 2. info.d and info.r is only updated in second
        ...
        for (const SelfCollisionInformation &info : selfInfos) {
          ...
          Utils::insertIntoTriplets<3, 3>(dr_df_delta, dr_dfiA, 3 * nA, 3 * nA); // drA_dfiA
          Utils::insertIntoTriplets<3, 3>(dr_df_delta, dr_dfiB, 3 * nA, 3 * nB);  // drA_dfiB
          Utils::insertIntoTriplets33(dr_df_delta, -dr_dfiA, 3 * nB, 3 * nA); // drB_dfiA
          Utils::insertIntoTriplets33(dr_df_delta, -dr_dfiB, 3 * nB, 3 * nB);  // drB_dfiB
          ...
          Utils::insertIntoTriplets(dr_df_delta, dr_dfprev, 3, threeM, 0, 0, 3 * nA, 0);
          Utils::insertIntoTriplets(dr_df_delta, negdr_dfprev, 3, threeM, 0, 0, 3 * nB, 0);
...
//      //Logging::logColor("gradient clipped at " + std::to_string(forwardInfo_new.stepIdx) + ": norm " + Utils::d2str(gradient_new.dL_dx.norm(), 4), //Logging::MAGENTA);
...
void Simulation::createClothMeshFromModel(std::vector<Vec3d> &particleIn, std::vector<Vec3i> &meshIn) {
  ...
  std::printf("Bounding box min:%s max:%s\n", Utils::vec2str(minDim, 3).c_str(), Utils::vec2str(maxDim, 3).c_str());
...
Simulation::createSystem(SceneConfiguration sceneConfig,
  ...
  Utils::checkFolderExistsAndCreate(OUTPUT_PARENT_FOLDER);
...
void Simulation::exportCurrentSimulation(std::string fileName) {
  ...
  Utils::checkFolderExistsAndCreate(std::string(SOURCE_PATH) + "/output/");
  Utils::checkFolderExistsAndCreate(parentFolder);
  ...
  for (int i = 0; i < forwardRecords.size(); i++) {
    ...
    Utils::checkFolderExistsAndCreate(fullFolder);
    ...
    areaTotalStr += "Frame " + std::to_string(i) + ":" + Utils::d2str(areaTotal, 7) + "\n";
  ...
  Utils::writeStringToFile(OUTPUT_PARENT_FOLDER + "area.txt", areaTotalStr);
  std::printf("finished\n");
...
void Simulation::exportOptimizationRecords(Demos demoId, std::string experimentNameGiven, bool useGiven) {
  std::string experimentName = useGiven ? experimentNameGiven : (sceneConfig.name + "-" + Utils::currentTimestampToStr());
  ...
  Utils::writeStringToFile(parentFolder + "task_info.txt", taskInfoToString(taskInfo));
  exportConfig(demoId, sceneConfig, parentFolder + "scene-config.txt");
  ...
  for (int i = 0; i < backwardOptimizationRecords.size(); i++) {
    ...
    Utils::checkFolderExistsAndCreate(iterationFolder + "/");
    exportSimulation(experimentName + "/iter" + std::to_string(i), backwardOptimizationRecords[i].first);
    Utils::writeStringToFile(iterationFolder + "/param.txt", parameterToString(taskInfo, backwardParam));
    Utils::writeStringToFile(iterationFolder + "/param.txt", "Actual:\n" + parameterToString(taskInfo, paramActual));

    Utils::writeStringToFile(iterationFolder + "/gradientInfo.txt",
                      backwrdInfoAndGradToString(taskInfo, backwardOptimizationRecords[i].second[0]));
    Utils::writeStringToFile(iterationFolder + "/forwardInfo.txt", forwardInfoToString(taskInfo,
...
void Simulation::exportStatistics(Demos demoId, Simulation::TaskSolveStatistics &statistics,
                                  Simulation::BackwardTaskInformation taskInfo, bool writePerf) {
  if (!statistics.configWritten) {
      std::string experimentName = sceneConfig.name + "-randseed-" + std::to_string(taskInfo.srandSeed)  + "-" + Utils::currentTimestampToStr() + "-forwardThresh-" +
                                 Utils::d2str(std::log10(taskInfo.forwardAccuracyLevel), 1);
  ...
  Utils::checkFolderExistsAndCreate(OUTPUT_PARENT_FOLDER);
  Utils::checkFolderExistsAndCreate(parentFolder);
  Utils::writeStringToFile(parentFolder + "iters.txt",
  ...
  for (int i = statistics.optimizationRecordsSaved; i < currentRecordSize; i++) {
    ...
    exportSimulation(iterationFolder,
                     backwardOptimizationRecords[i].first);
    Utils::writeStringToFile(OUTPUT_PARENT_FOLDER +  iterationFolder + "/param.txt",
  ...
  Utils::checkFolderExistsAndCreate(lastFrameMeshFolder);
  ...
  if (writePerf) {
    ...
    out += "Demo Name:" + DEMOS_STRINGS[(int)demoId] + "\n";
    ...
    for (int i = 0; i < statistics.completeForwardLog.size(); i++) {
      ...
      out += "iter" + std::to_string(i) + ":" + Utils::d2str(info.totalRuntime / 1000000.0, 8) + "\n";
    }
    ...
    out += "Total Forward Time:" + Utils::d2str(totalForwardTime / 1000000.0, 6) + "\n";
    out += "Total Backward Time:" + Utils::d2str(totalBackwardTime / 1000000.0, 6) + "\n";
    out += "Total Time:" + Utils::d2str((totalForwardTime + totalBackwardTime) / 1000000.0, 6) + "\n";
    Utils::writeStringToFile(
            OUTPUT_PARENT_FOLDER + statistics.experimentName + subFolder +
            "perf.txt", out);
  }
...
void Simulation::exportConfig(Demos demoId, Simulation::SceneConfiguration &config, std::string fileName) {
  std::ofstream myfile;
  std::printf("writing config to file...%s.txt\n", fileName.c_str());
  myfile.open(fileName);
  myfile << "demoName:" << config.name << " " << "\n";
  myfile << "demoId:" << int(demoId) << " " << "\n";
  myfile << "SceneBbox: min:" << Utils::vec2str(config.sceneBbox.min, 4) << " max:"
         << Utils::vec2str(config.sceneBbox.max, 4) + "\n";
  myfile << "FPS:" << ((int) std::round(1.0f / config.timeStep)) << "\n";
  myfile << "Frame Number:" << config.stepNum + 1 << "\n";
  myfile << "CameraPos:" << Utils::vec2str(config.camPos, 5) << "\n";
  //Vec3d lookAtPos = Utils::toEigen(getLookAtPos(this, config));
  //myfile << "CameraLookAtPos:" << Utils::vec2str(lookAtPos, 5) << "\n";
  ...
  for (std::pair<std::string, Vec3d> &nameColor : objNameAndColor) {
    myfile << nameColor.first << " color:" << Utils::vec2str(nameColor.second) << "\n";
...
std::string Simulation::taskInfoToString(const Simulation::BackwardTaskInformation &taskInfo) {
  ...
  out += "Forward Accuracy:" + Utils::d2str(taskInfo.forwardAccuracyLevel, 11) + "\n";
  out += "Backward Accuracy:" + Utils::d2str(taskInfo.backwardAccuracyLevel, 11) + "\n";
...
Simulation::parameterToString(const Simulation::BackwardTaskInformation &taskInfo, Simulation::ParamInfo paramGuess) {
  ...
  for (int i = 0; i < Constraint::ConstraintType::CONSTRAINT_NUM; i++) {
    if (taskInfo.dL_dk_pertype[i]) {
      out += "k_" + CONSTRAINT_TYPE_STRINGS[i] + ":" + Utils::d2str(paramGuess.k_pertype[i], 6) + "\n";
    }
  }

  if (taskInfo.dL_density) {
    out += "density:" + Utils::d2str(paramGuess.density, 6) + "\n";
  }

  if (taskInfo.dL_dmu) {
    for (std::pair<int, double> pair : paramGuess.mu) {
      out += "mu from prim" + std::to_string(pair.first) + ":" + Utils::d2str(pair.second, 6) + "\n";

    }
  }

  if (taskInfo.dL_dx0) {
    out += "x0: norm:" + Utils::d2str(paramGuess.x0.norm(), 5) + "\n";
    for (int i = 0; i < paramGuess.x0.rows() / 3; i += 200) {
      out += "pId-" + std::to_string(i) + Utils::vecXd2str(paramGuess.x0.segment(i * 3, 3), 5) + "\n";

    }
  }

  if (taskInfo.dL_dfext) {
    out += "f_ext:" + Utils::vec2str(paramGuess.f_ext, 6) + "\n";
  }

  if (taskInfo.dL_dfwind) {
    out += "f_wind:" + Utils::vec2str(paramGuess.f_extwind, 6) + "\n";
  }
    ...
    for (int matSetIdx = 0; matSetIdx < paramGuess.controlPointSplines.size(); matSetIdx++) {
      ...
      for (int i = 0; i < paramGuess.controlPointSplines[matSetIdx].size(); i++) {
        ...
        for (Spline::Segment &s : spline.segments) {
          out += "t[" + Utils::d2str(s.startFraction) + "," + Utils::d2str(s.endFraction) + "]:";
          out += "p1:" + Utils::vec2str(spline.segments[0].p1, 6);

          switch (spline.type) {
            case Spline::SplineType::ENDPOINT_AND_UP: {
              out += ",up:" + Utils::d2str(spline.segments[0].yUp, 4);
              break;
            }
            case Spline::SplineType::ENDPOINT_AND_TANGENTS: {
              out += ",m0:" + Utils::vec2str(spline.segments[0].m0, 4);
              out += ",m1:" + Utils::vec2str(spline.segments[0].m1, 4);
              break;
            }
...
Simulation::backwrdInfoAndGradToString(const Simulation::BackwardTaskInformation &taskInfo,
  ...
  out += "Loss:" + Utils::d2str(grad.loss, 3) + "\n";
  ...
  out += "Backward Total Runtime[ms]:" + Utils::d2str(grad.totalRuntime / 1000.0, 7) + "\n";
  ...
  for (TimerEntry &breakdown: grad.accumTimer) {
    ...
    out += label + "[ms]:" + Utils::d2str(duration / 1000.0, 5) + "\t\t|" + Utils::d2str(percentage * 100, 3) + "%\n";
  }
  ...
  out += "Direct-Solve[ms]:" + Utils::d2str(grad.accumSolvePerformanceReport.solveDirectMicroseconds / 1000.0, 5) + "\t|" +
         Utils::d2str(percentageDirect * 100, 3) + "%\n";
  out += "Iter-Solve[ms]:" + Utils::d2str(grad.accumSolvePerformanceReport.solveIterativeMicroseconds / 1000.0, 5) +
         "\t|" +
         Utils::d2str(percentageIter * 100, 3) + "%\n";


  out += "\n============Gradient Info:======================\n";
  for (int i = 0; i < Constraint::ConstraintType::CONSTRAINT_NUM; i++) {
    if (taskInfo.dL_dk_pertype[i]) {
      out += "dL/dk_" + CONSTRAINT_TYPE_STRINGS[i] + ":" + Utils::d2str(grad.dL_dk_pertype[i], 5) + "\n";
    }
  }

  if (taskInfo.dL_density) {
    out += "dL/ddensity:" + Utils::d2str(grad.dL_ddensity, 5) + "\n";
  }

  if (taskInfo.dL_dmu) {
    for (std::pair<int, double> pair : grad.dL_dmu) {
      out += "dL_dmu_" + std::to_string(pair.first) + ":" + Utils::d2str(pair.second, 5) + "\n";

    }
  }

  if (taskInfo.dL_dfext) {
    out += "dL/df_ext:" + Utils::vec2str(grad.dL_dfext, 4) + "\n";
  }

  if (taskInfo.dL_dfwind) {
    out += "dL/df_wind:" + Utils::vec2str(grad.dL_dwind, 4) + "\n";
  }

  if (taskInfo.dL_dx0) {
    out += "dL/dx0: norm:" + Utils::d2str(grad.dL_dx.norm(), 5) + "\n";
    for (int i = 0; i < grad.dL_dx.rows() / 3; i += 200) {
      out += "pId-" + std::to_string(i) + Utils::vecXd2str(grad.dL_dx.segment(i * 3, 3), 5) + "\n";
  ...
  if (taskInfo.dL_dcontrolPoints) {
    for (int matSetIdx = 0; matSetIdx < grad.dL_dsplines.size(); matSetIdx++) {
      ...
      for (int i = 0; i < grad.dL_dsplines[matSetIdx].size(); i++) {
        ...
        out += "dL_dp1:" + Utils::vecXd2str(dL_dspline.segment(0, 3), 4);

        switch (type) {
          case Spline::SplineType::ENDPOINT_AND_UP: {
            out += ",dL_dup:" + Utils::d2str(dL_dspline[3], 4);
            break;
          }
          case Spline::SplineType::ENDPOINT_AND_TANGENTS: {
            out += ",dL_dm0:" + Utils::vecXd2str(dL_dspline.segment(3, 3), 4);
            out += ",dL_dm1:" + Utils::vecXd2str(dL_dspline.segment(6, 3), 4);
            break;
          }
