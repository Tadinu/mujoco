...
// first create axis aligned mesh
  ...
  rotationFromParent = Utils::axisToRotation(axis, initialDir);

  this->globalAxis = rotationFromParent * parentAxis;
  globalRotation = Utils::axisToRotation(this->globalAxis, initialDir);
  ...
  auto getCapsulePos = [&](double radius, double phi, double theta, bool withRotation = true) {
      ...
      double z = 0;//radius * glm::sin(glm::radians(phi)) * glm::sin(glm::radians(theta));
      double y = 0;//radius * glm::cos(glm::radians(180 - theta));
