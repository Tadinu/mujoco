...
void BackwardTaskSolver::setDemoSceneConfigAndConvergence(Simulation* system, Demos demoId, Simulation::BackwardTaskInformation &taskInfo) {


  system->sceneConfig = OptimizationTaskConfigurations::demoNumToConfigMap[demoId].scene;
...
void BackwardTaskSolver::resetSplineConfigsForControlTasks(Demos demoId, Simulation *system,
...
void BackwardTaskSolver::setLossFunctionInformationAndType(LossType &lossType, Simulation::LossInfo &lossInfo,
                                                           Simulation *system, Demos demoId) {
    ...
    lossType = OptimizationTaskConfigurations::demoNumToConfigMap[demoId].lossType;
  switch (demoId) {
    case Demos::DEMO_WEAR_HAT: {
      Vec3d bustCenter =
    ...
    case Demos::DEMO_DRESS_TWIRL: {
      lossInfo.targetTwirlHeight = 0.3;
      system->sceneConfig.name = "dressTwirl-" +  Utils::d2str(lossInfo.targetTwirlHeight, 5);
      lossInfo.loopPoints = {};
...
void BackwardTaskSolver::setInitialConditions(Demos demoId, Simulation *system,
                                              Simulation::ParamInfo &paramGroundtruth,
                                              Simulation::BackwardTaskInformation &taskInfo) {
  switch (demoId) {
    case Demos::DEMO_WIND: {
    ...
    case Demos::DEMO_WEAR_SOCK:
 {
      ...
      resetSplineConfigsForControlTasks(demoId, system,  paramGroundtruth);
      break;
    }
