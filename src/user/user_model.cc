...
template <class T>
T* mjCModel::AddObject(vector<T*>& list, const char* type) {
  T* obj = new T(this);
...
template <class T>
T* mjCModel::AddObjectDef(vector<T*>& list, const char* type, mjCDef* def) {
...
void mjCModel::TryCompile(mjModel*& m, mjData*& d, const mjVFS* vfs) {
  ...
  for (int i=0; i<meshes.size(); i++) {
    std::cout << "compile mesh " << meshes[i]->name << " userverts num " << meshes[i]->get_uservert().size() << std::endl;
